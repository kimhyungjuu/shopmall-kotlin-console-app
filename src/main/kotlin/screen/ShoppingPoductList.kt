...
class ShoppingProductList(private val selectedCategory: String) : Screen() {
    ...
    /*
     *  사용자가 입력한 카테고리 정보를 받아 해당 카테고리 상품을 출력
     */
    fun showProducts() {
        // 스텍에 저장
        ...
        if (!categoryProducts.isNullOrEmpty()) {
            ...
            // 장바구니에 담을 상품 선택
            showCartOptions(categoryProducts)
        } else {
            showEmptyProductMessage(selectedCategory)
        }
    }

    private fun showCartOptions(categoryProducts: List<Product>) {
        println("""
            $LINE_DIVIDER
        ...
        categoryProducts.getOrNull(selectedIndex)?.let{
            ...
            println("$selectedIndex 은 목록에 없는 상품입니다. 다시 입력해 주세요")
            showProducts()
        }
    }
