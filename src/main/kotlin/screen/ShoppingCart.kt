...
class ShoppingCart : Screen() {
    ...
    private fun moveToPreviousScreen() {
        // (1) 스텍에서 현재화면을 popup시킴
        ScreenStack.pop()
        
        // (2) 현재 스텍에서 제일 위의 클래스를 확인 후
        //      show...() 메소드를 실행
        when( val previousScreen = ScreenStack.peek()) {
            is ShoppingCategory -> {
                previousScreen.showCategories()     // selectedCategories 필요
            }
            is ShoppingProductList -> {
                previousScreen.showProducts()       // selectedCategories 필요
            }
